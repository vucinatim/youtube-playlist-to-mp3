...
def download_audio():
    ...
    try:
        ...
        print(f"[Flask] Time for extract_info: {t_after_extract - t_before_extract:.2f}s")
        ...
        if not chosen_format or not chosen_format.get('url'):
            ...
            return jsonify({"error": "Could not find a suitable audio format URL."}), 500
        t_after_format = time.time()
        print(f"[Flask] Time for format selection: {t_after_format - t_before_format:.2f}s")
        # --- End Format Selection ---

        download_url = chosen_format['url']
        file_ext = chosen_format.get('ext', 'mp3')
        content_type = chosen_format.get('http_headers', {}).get('Content-Type', 'audio/mpeg')
        content_length_est = str(chosen_format.get('filesize') or chosen_format.get('filesize_approx') or 0)

        print(f"[Flask] Found format URL for {file_ext}, estimated size: {content_length_est}")
        ...
        req = requests.get(download_url, stream=True, headers={
            'User-Agent': chosen_format.get('http_headers', {}).get('User-Agent', 'Mozilla/5.0'),
            'Accept': '*/*',
        ...
        print(f"[Flask] Time for requests.get (headers): {t_after_request_headers - t_before_request:.2f}s")

        final_content_length = req.headers.get('Content-Length', content_length_est)
        print(f"[Flask] Streaming {final_content_length} bytes...")
        ...
        def generate():
            ...
            print(f"[Flask] Time for streaming loop: {t_after_stream - t_before_stream:.2f}s")
            t_end = time.time()
        ...
        response.headers['Content-Disposition'] = f'attachment; filename="audio.{file_ext}"'
